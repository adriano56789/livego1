...
export default function ContributionRankingModal({ onClose, liveRanking }: ContributionRankingModalProps) {
    ...
    useEffect(() => {
        const fetchRanking = async () => {
            if (activeTab === 'Live') {
                // Use liveRanking prop passed from parent (Live Room) which has real-time data
                // Map User type to RankedUser for consistency if needed, though they overlap
                setRankingData(liveRanking as RankedUser[]); 
                return;
            }
            ...
            try {
                ...
                switch(activeTab) {
                    case 'Diária':
                        data = await api.getDailyRanking();
                        break;
                    case 'Semanal':
                        data = await api.getWeeklyRanking();
                        break;
                    case 'Mensal':
                        data = await api.getMonthlyRanking();
                        break;
                }
                setRankingData(data || []);
            } catch (error) {
                console.error(`Failed to load ${activeTab} ranking`, error);
            } finally {
                setIsLoading(false);
            }
        };
    ...
    // Ensure sorted
    const sortedData = [...rankingData].sort((a, b) => (b.value || 0) - (a.value || 0));
    ...
    const renderEmptyState = () => (
        <div className="flex flex-col items-center justify-center mt-20 opacity-50">
            <div className="w-24 h-24 rounded-full bg-[#2C2C2E] flex items-center justify-center mb-4">
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-transparent" onClick={onClose}>
            <div 
                className="w-full bg-[#18181b] rounded-t-3xl h-[65vh] flex flex-col animate-in slide-in-from-bottom duration-300 relative shadow-2xl border-t border-white/5"
            ...
            >
                ...
                <div className="flex px-6 border-b border-white/5">
                    {tabs.map((tab) => (
                        <button
                            ...
                            className={`flex-1 pb-3 text-sm font-medium relative transition-colors ${activeTab === tab ? 'text-white font-bold' : 'text-gray-500'}`}
                        >
                            {tab}
                            {activeTab === tab && (
                                <div className="absolute bottom-0 left-1/2 -translate-x-1/2 w-8 h-[3px] bg-[#FFD700] rounded-full"></div>
                            )}
                        </button>
                    ))}
                </div>
                ...
                <div className="flex-1 overflow-y-auto px-4 pb-6 scrollbar-hide">
                    ...
                    ) : sortedData.length === 0 ? renderEmptyState() : (
                        <div className="flex flex-col items-center pt-6">
                            ...
                            {topUser && (
                                <div className="relative flex flex-col items-center mb-8 w-full">
                                    ...
                                    <div className="absolute -top-10 z-10">
                                        <CrownIcon className="w-12 h-12 text-[#FFD700] fill-[#FFD700]" />
                                    </div>
                                    ...
                                    <div className="relative mb-2">
                                        ...
                                        <div className="absolute -bottom-3 left-1/2 -translate-x-1/2 bg-[#FFD700] text-black text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg border-2 border-[#18181b]">
                                            TOP 1
                                        </div>
                                    </div>
                            ...
                            <div className="w-full flex flex-col gap-1">
                                {otherUsers.map((user, index) => {
                                    const rank = index + 2;
                                    const rankBg = rank === 2 ? 'bg-[#C0C0C0] text-black' : (rank === 3 ? 'bg-[#CD7F32] text-black' : 'text-gray-500 font-medium');
                                    const isTop3 = rank <= 3;
                                    ...
                                    return (
                                        <div key={user.id || index} className="flex items-center justify-between p-3 rounded-xl hover:bg-white/5 transition-colors">
                                            <div className="flex items-center gap-3">
                                                {/* Rank Number */}
                                                <div className={`w-6 h-6 rounded-full flex items-center justify-center text-xs font-bold ${isTop3 ? rankBg : ''}`}>
                                                    {rank}
                                                </div>
                                            ...
                                            <div className="flex items-center gap-1">
                                                <span className="text-[#FFD700] font-bold text-sm">{((user.value || 0) / 1000).toFixed(0)}K</span>
                                                <SolidDiamondIcon className="w-3 h-3 text-[#FFD700]" />
                                            </div>
                                        </div>
                                    );
                                })}
