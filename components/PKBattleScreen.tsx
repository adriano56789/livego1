...
import { GiftIcon, MessageIcon, SendIcon, MoreIcon, CloseIcon, PlusIcon, ViewerIcon, StarIcon, HeartIcon, GoldCoinWithGIcon, BellIcon, FanClubHeaderIcon, SmileIcon, ArrowUpIcon } from './icons';
...
export const PKBattleScreen: React.FC<PKBattleScreenProps> = ({ 
    ...
    const myProgress = totalScore > 0 ? ((myScore || 0) / totalScore) * 100 : 50;
    ...
    const handleSendGift = async (gift: Gift, quantity: number): Promise<User | null> => {
        ...
        setMyScore(prev => (prev || 0) + totalCost);
    ...
    useEffect(() => {
        ...
        const handleNewGift = (payload: GiftPayload) => {
            ...
            if (payload.fromUser.id === currentUser.id) return;
    ...
    return (
        ...
        >
            <div className="h-[65%] w-full relative" onClick={handleHeartClick}>
                <div className="absolute inset-0 grid grid-cols-2">
                    ...
                    <div className="h-full w-full bg-gray-800 relative cursor-pointer" onClick={(e) => { e.stopPropagation(); onViewProfile(opponent); }}>
                        ...
                        {!opponentIsFollowed && (
                            <div className={`absolute top-12 right-0 p-4 transition-opacity duration-300 ${isUiVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                                <button 
                ...
                <header className={`p-3 bg-transparent absolute top-0 left-0 right-0 z-20 transition-opacity duration-300 ${isUiVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                                ...
                                <div className="flex flex-col space-y-2">
                                    ...
                                    <div className="flex items-center space-x-2 pl-1">
                                        <button onClick={(e) => { e.stopPropagation(); setIsRankingOpen(true); }} className="flex items-center bg-black/40 rounded-full px-2 py-1 space-x-1 text-xs cursor-pointer">
                                            ...
                                            <span className="text-white font-semibold">{(myScore ?? 0).toLocaleString()}</span>
                                        </button>
                                    </div>
                                </div>
                            </div>
                    ...
                    <div className={`w-full px-4 absolute top-24 left-0 right-0 z-10 transition-opacity duration-300 ${isUiVisible ? 'opacity-100' : 'opacity-0 pointer-events-none'}`}>
                      <div className="relative w-full h-3 bg-red-600 rounded-full overflow-hidden">
                        <div className="absolute top-0 left-0 h-full bg-blue-600 transition-all duration-500" style={{ width: `${myProgress}%` }}></div>
                      ...
                      <div className="flex justify-between mt-1.5">
                        <div className="flex items-center space-x-1.5">
                            <HeartIcon className="w-5 h-5 text-pink-400" fill="currentColor" />
                            <span className="font-bold text-white score-pop">{(myScore ?? 0).toLocaleString()}</span>
                            <span className="font-bold text-white score-pop text-sm ml-2">({myHearts || 0})</span>
                        </div>
                        ...
                        <div className="flex items-center space-x-1.5">
                            <span className="font-bold text-white score-pop text-sm mr-2">({opponentHearts || 0})</span>
                            <HeartIcon className="w-5 h-5 text-blue-400" fill="currentColor" />
                            <span className="font-bold text-white score-pop">{(opponentScore ?? 0).toLocaleString()}</span>
                        </div>
                      </div>
                    </div>
            ...
            {isOnlineUsersOpen && <OnlineUsersModal onClose={() => setOnlineUsersOpen(false)} users={onlineUsers} streamId={streamer.id} />}
