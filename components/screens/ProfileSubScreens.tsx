    ...
    useEffect(() => {
        const loadTopFans = async () => {
            ...
            try {
                ...
                const data = await api.getTopFans();
                setFans(data || []);
            } catch (error) {
        ...
        <div className="fixed inset-0 bg-[#0A0A0A] z-50 flex flex-col font-sans animate-in slide-in-from-right duration-200">
            ...
            <div className="flex-1 overflow-y-auto p-4 bg-[#0A0A0A]">
                {loading ? (
                    ...
                    fans.map((fan, index) => (
                        <div key={fan.id} className="flex items-center bg-[#1C1C1E] p-3 rounded-lg mb-2 border border-white/5">
                             {/* Rank */}
...
export const BlockListScreen: React.FC<ScreenProps> = ({ onClose }) => {
    ...
    useEffect(() => {
        const loadBlockList = async () => {
            setLoading(true);
            try {
                /* Fix: Changed getBlockedUsers to getBlocklist to match available method in services/api.ts */
                // Fetch real blocklist from API
                const users = await api.getBlocklist();
