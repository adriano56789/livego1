...
const MessagesScreen: React.FC = () => {
    ...
    const handleBlockUser = async (userToBlock: User) => {
        try {
            ...
            if (result.success) {
                // Idealmente, você usaria uma função addToast aqui
                console.log(`${userToBlock.name} bloqueado com sucesso.`);
    ...
    if (selectedUser) {
        return <PrivateChatScreen user={selectedUser} onClose={() => { setSelectedUser(null); loadMessagesData(); }} onBlock={handleBlockUser} variant="page" />;
    }
