...
interface UserProfileDetailScreenProps {
    currentUser: User; // This is the user TO BE VIEWED
    onClose: () => void;
    onOpenFans: () => void;
            ...
            <div className="pb-4">
                ...
                <div className="flex flex-col">
                    {reasons.map((reason, idx) => (
                        <button key={idx} className="text-left px-4 py-4 text-white text-sm border-b border-white/5 hover:bg-white/5 active:bg-white/10">{reason}</button>
                    ))}
                </div>
            </div>
        </ModalWrapper>
...
// --- Main Component ---
export default function UserProfileDetailScreen({ currentUser, onClose, onOpenFans, onOpenFollowing, onOpenTopFans, onChat, onFollow, addToast, onUpdateUser }: UserProfileDetailScreenProps) {
    const [activeTab, setActiveTab] = useState<'obras' | 'curtidas' | 'detalhes'>('obras');
    const [showOptions, setShowOptions] = useState(false);
    const [showEditProfile, setShowEditProfile] = useState(false);
    const [showReportModal, setShowReportModal] = useState(false);
    
    // State for user data (to support updates)
    const [userData, setUserData] = useState<User & { isFollowing?: boolean }>(currentUser);
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                // FIX: Property 'getById' does not exist on type 'api.users'. Changed to 'get'.
                const fetchedUser = await api.users.get(currentUser.id);
    ...
    const handleFollowClick = async () => {
        try {
            ...
            setUserData(prev => ({
                ...
                fans: (prev.fans || 0) + (prev.isFollowing ? -1 : 1)
            }));

            /* Fix: Changed api.followUser to api.toggleFollow as followUser does not exist in api.ts */
            // FIX: Property 'follow' does not exist on type 'api.users'. Changed to 'toggleFollow'.
            await api.users.toggleFollow(userData.id);
            if (onFollow) onFollow(); // Notify parent if needed
        } catch (error) {
            // Revert on error
            setUserData(prev => ({
                ...prev,
            ...
            if (addToast) addToast(ToastType.Error, "Falha ao seguir usuário.");
        }
    };

    const handleSaveProfile = async (updatedData: Partial<User>) => {
        try {
            const { success, user: updatedUserFromApi } = await api.users.update(userData.id, updatedData);
            if (success && updatedUserFromApi) {
                const newUserData = { ...userData, ...updatedUserFromApi };
                setUserData(newUserData as User & { isFollowing?: boolean });
                if(addToast) addToast(ToastType.Success, "Perfil atualizado!");

                // Se for o perfil do usuário logado, atualiza o estado global
                if (canEdit && onUpdateUser) {
                    onUpdateUser(newUserData as User);
                }
            } else {
                 throw new Error("API returned failure");
            }
        } catch(e) {
            if(addToast) addToast(ToastType.Error, "Falha ao atualizar perfil.");
            // Refetch data on failure to revert optimistic updates
            const fetchedUser = await api.users.get(currentUser.id);
            if (fetchedUser) {
                setUserData(fetchedUser as User & { isFollowing?: boolean });
            }
    ...
    const canEdit = userData.id === 'me' || !onFollow; // onFollow is usually passed when viewing OTHERS
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black">
            ...
            >
                ...
                <div className="h-40 w-full relative shrink-0">
                    ...
                    <div className="absolute top-4 right-4 z-10 flex gap-3">
                        {canEdit ? (
                             <button onClick={() => setShowEditProfile(true)} className="w-9 h-9 flex items-center justify-center bg-black/30 backdrop-blur-md rounded-full hover:bg-black/50 transition-colors">
                ...
                {/* Footer Actions (Only for Visitors) */}
                {onFollow && (
            ...
            {showEditProfile && <EditProfileScreen user={userData} onClose={() => setShowEditProfile(false)} onSave={handleSaveProfile} addToast={addToast} />}
