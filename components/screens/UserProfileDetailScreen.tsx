...
// --- Main Component ---
export default function UserProfileDetailScreen({ currentUser, onClose, onOpenFans, onOpenFollowing, onOpenTopFans, onChat, onFollow, addToast }: UserProfileDetailScreenProps) {
    ...
    useEffect(() => {
        ...
        const fetchUserData = async () => {
            try {
                const fetchedUser = await api.getUser(currentUser.id);
    ...
    const handleFollowClick = async () => {
        try {
            ...
            // Call API
            const result = await api.followUser('me', userData.id);
    ...
    const handleSaveProfile = async (updatedData: Partial<User>) => {
        try {
            const { success, user } = await api.updateProfile(userData.id, updatedData);
            if (success && user) {
    ...
    return (
        <div className="fixed inset-0 z-[100] bg-black">
                            ...
                            <div className="p-4 space-y-4">
                                <div className="bg-[#1C1C1E] rounded-xl p-5 space-y-4 border border-white/5">
                                    ...
                                    <div className="flex justify-between text-xs items-center">
                                        ...
                                        <span className="text-gray-300 font-medium">20/10/2023</span>
                                    </div>
                                </div>
                            </div>
                        )}
            ...
            {showOptions && <OptionsModal onClose={() => setShowOptions(false)} onBlock={() => setShowOptions(false)} onReport={() => setShowReportModal(true)} />}
            {showEditProfile && <EditProfileScreen user={userData} onClose={() => setShowEditProfile(false)} onSave={handleSaveProfile} />}
