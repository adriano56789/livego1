...
const WalletScreen: React.FC<WalletScreenProps> = ({ onClose, initialTab = 'Diamante', currentUser, updateUser, addToast }) => {
    ...
    const [historyFilter, setHistoryFilter] = useState('Cancelado'); 
    const [selectedMethod, setSelectedMethod] = useState<'pix' | 'mercadopago'>('pix');
    const [pixKey, setPixKey] = useState('teste.saque@exemplo.com');
    ...
    const handleConfirmWithdraw = async () => {
        ...
        try {
            const { success, message, record } = await api.requestWithdraw(amount, currentUser.withdrawal_method);
            if (success) {
    ...
    const handleSaveWithdrawalMethod = async () => {
        if (!currentUser) return;
        try {
            const { success, user } = await api.setWithdrawalMethod(selectedMethod, { key: pixKey });
            if (success && user) {
                updateUser(user);
    ...
    const handlePurchaseComplete = async () => {
        ...
        try {
            const { success, user } = await api.buyDiamonds(currentUser.id, selectedPackage.diamonds, selectedPackage.price);
            if (success && user) {
                updateUser(user);
        ...
        return (
            <div className="fixed inset-0 bg-[#0A0A0A] z-[120] flex flex-col font-sans">
                <div className="flex items-center justify-between p-4 bg-[#0A0A0A]">
                    ...
                    <h2 className="text-white text-lg font-bold">Histórico de Compras</h2>
                    <div className="w-6" />
                </div>
                ...
                <div className="flex-1 flex flex-col items-center justify-center p-4">
                     <p className="text-gray-500 text-sm">Nenhum histórico de compras.</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="fixed inset-0 bg-[#0A0A0A] z-[120] flex flex-col font-sans animate-in slide-in-from-right duration-300">
            ...
            <div className={`flex-1 p-4 bg-[#0A0A0A] ${activeTab === 'Ganhos' ? 'overflow-hidden' : 'overflow-y-auto'}`}>
                {activeTab === 'Diamante' ? (
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="space-y-2">
                            ...
                            {withdrawAmount && !isNaN(Number(withdrawAmount)) && (
                                <div className="text-sm text-gray-400 mt-1">
                                    Valor Bruto: {formatCurrency(calculation?.gross_value || 0)}
                                </div>
                            )}
                        </div>
                        ...
                        <div className="space-y-3 pt-2">
                            <div className="flex justify-between items-center text-sm">
                                ...
                                <span className="text-white font-medium">{withdrawAmount ? withdrawAmount : '0'}</span>
                            </div>
                            <div className="flex justify-between items-center text-sm">
                            ...
                            <div className="flex justify-between items-center text-base pt-2 border-t border-white/10">
                                ...
                                <span className="text-white font-bold text-[#22c55e]">{isCalculating && withdrawAmount ? '...' : formatCurrency(displayData.net_value)}</span>
                            </div>
                        </div>
