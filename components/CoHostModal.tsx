...
interface CoHostModalProps {
  ...
  streamId: string;
  allUsers?: User[]; // Optional prop to pass all users if needed
}
...
const CoHostModal: React.FC<CoHostModalProps> = ({ isOpen, onClose, onInvite, onOpenTimerSettings, currentUser, addToast, streamId }) => {
  ...
  useEffect(() => {
    // Prevent background scrolling and handle touch moves outside modal
  ...
  const handleInviteClick = async (friend: User) => {
    ...
    try {
      ...
      if (success) {
        ...
        // This triggers the redirect to PKBattleScreen in the parent component
        onInvite(friend); 
      } else {
  ...
  const handleQuickInvite = async (quickFriend: QuickCompleteFriend) => {
    ...
    let friendToInvite = friends.find(f => f.id === quickFriend.id);
    
    // Se não encontrar (comum em mocks ou listas parciais), cria um objeto User temporário
    // para garantir que o fluxo de convite e redirecionamento funcione
    if (!friendToInvite) {
        friendToInvite = {
            id: quickFriend.id,
            identification: quickFriend.id,
            name: quickFriend.name,
            avatarUrl: `https://i.pravatar.cc/150?u=${quickFriend.id}`,
            coverUrl: `https://picsum.photos/seed/${quickFriend.id}/800/600`,
            diamonds: 0,
            level: 1,
            xp: 0,
            isLive: false,
    ...
    // Chama a função principal de convite que lida com API e Redirecionamento
    await handleInviteClick(friendToInvite);

    // Atualiza visualmente o status da tarefa rápida
    try {
        const { success } = await api.completeQuickFriendTask(quickFriend.id);
        if (success) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Main content */}
        <div 
            className="flex-grow p-4 space-y-4 overflow-y-auto scrollbar-hide outline-none"
