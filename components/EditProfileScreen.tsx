            ...
            <div className="p-4">
                ...
                <input 
                    ...
                    onChange={(e) => {
                        ...
                        if (!isNaN(d.getTime())) {
                            const formatted = `${String(d.getDate() + 1).padStart(2, '0')}/${String(d.getMonth() + 1).padStart(2, '0')}/${d.getFullYear()}`;
                            setDate(formatted);
                        }
                    }}
...
const EditProfileScreen: React.FC<EditProfileScreenProps> = ({ user, onClose, onSave }) => {
    ...
    const [formData, setFormData] = useState({
        name: user.name,
        gender: user.gender,
        birthday: '15/07/1992', // Mock initial
        bio: user.bio || 'Olá! Bem-vindo ao meu perfil.',
        location: user.location || 'Não especificado',
        emotionalState: 'Não especificado',
        tags: 'Não especificado',
        profession: 'Não especificado'
    });
    const [activeModal, setActiveModal] = useState<string | null>(null);
    const photos = [
        { id: 1, url: 'https://picsum.photos/seed/p1/200', isMain: true },
        { id: 2, url: 'https://picsum.photos/seed/p2/200', isMain: false },
        { id: 3, url: 'https://picsum.photos/seed/p3/200', isMain: false },
    ];
    const menuItems = [
        { id: 'name', label: 'Apelido', value: formData.name },
        { id: 'gender', label: 'Gênero', value: formData.gender === 'male' ? 'Masculino' : 'Feminino' },
    ...
    return (
        <div ref={scrollRef} className="fixed inset-0 z-[140] bg-[#121212] flex flex-col font-sans animate-in slide-in-from-right duration-300 overflow-y-auto scrollbar-hide outline-none overflow-x-hidden w-full max-w-[100vw]" tabIndex={0}>
            <div className="flex items-center justify-between px-4 py-3 border-b border-white/5 bg-[#121212] sticky top-0 z-10 w-full">
                ...
                <button onClick={handleSaveProfile} className="text-[#a855f7] font-bold text-sm">Salvar</button>
            </div>
            <div className="flex-1 pb-10 w-full">
                ...
                <div className="px-4 mt-4 mb-6 w-full">
                    <div className="flex gap-3 overflow-x-auto scrollbar-hide pb-2 w-full">
                        {photos.map((photo) => (
                            <div key={photo.id} className="relative w-24 h-24 flex-shrink-0 group">
                                <img src={photo.url} className="w-full h-full object-cover rounded-xl" />
                                <button className="absolute top-1 right-1 bg-black/60 rounded-full p-1 opacity-0 group-hover:opacity-100 transition-opacity"><TrashIcon className="w-3 h-3 text-white" /></button>
                                {photo.isMain && <div className="absolute bottom-1 left-1 bg-black/60 px-1.5 py-0.5 rounded text-[9px] text-white font-bold">Retrato</div>}
                            </div>
                        ))}
                        <div className="w-24 h-24 bg-[#1C1C1E] rounded-xl flex items-center justify-center border border-white/10 flex-shrink-0 cursor-pointer hover:bg-[#2C2C2E] transition-colors">
                            <PlusIcon className="w-8 h-8 text-gray-500" />
                        </div>
                    </div>
                    <p className="text-gray-500 text-[10px] mt-1">3/8 Clique para ver e apagar imagens, segure para ordenar</p>
                </div>
