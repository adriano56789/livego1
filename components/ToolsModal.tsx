...
import { 
    ShareIcon, MagicIcon, SlidersIcon, MicIcon, MicOffIcon, 
...
const ToolItem = ({ icon: Icon, label, onClick, isActive = false, activeColor = "bg-green-500 text-white", defaultColor = "bg-[#2C2C2E] text-gray-400", hasDot = false }: any) => (
    <button onClick={onClick} className="flex flex-col items-center gap-2 p-1 active:scale-95 transition-transform w-full">
...
const ToolsModal: React.FC<ToolsModalProps> = ({ 
    ...
    onToggleAutoPrivateInvite, isAutoPrivateInviteEnabled, onOpenPrivateChat, onOpenEditStreamInfo
}) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-end justify-center bg-transparent" onClick={onClose}>
            ...
            >
                ...
                <div className="mb-6">
                    <h4 className="text-gray-500 text-xs font-medium mb-4 px-1">Ferramentas de Interação</h4>
                ...
                <div>
                    <h4 className="text-gray-500 text-xs font-medium mb-4 px-1">Ferramentas de âncora</h4>
                    <div className="grid grid-cols-4 gap-y-6 gap-x-4">
                        ...
                        <ToolItem 
                            icon={!isMicrophoneMuted ? MicIcon : MicOffIcon} 
                            label="Microfone" 
                        ...
                        <ToolItem 
                            icon={!isSoundMuted ? VolumeIcon : VolumeOffIcon} 
                            label="Som" 
                        ...
                        <ToolItem 
                            icon={SunIcon} // Using Sun for clarity/brightness
                            label="Clareza" 
                            onClick={onOpenClarityPanel} 
                            defaultColor="bg-[#2C2C2E] text-white"
                        />
                        ...
                        <ToolItem 
                            icon={UserPlusIcon} 
                            label="Auto Convite" 
                            isActive={isAutoPrivateInviteEnabled}
                            onClick={onToggleAutoPrivateInvite} 
                            defaultColor="bg-[#2C2C2E] text-white"
                        />
